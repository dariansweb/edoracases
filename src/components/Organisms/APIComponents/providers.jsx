...
const ProviderCard = ({ provider }) => {
  ...
  return (
    <div className="provider-info-card">
      ...
      <p>
        <strong>Coverage:</strong> {provider.coverage.district}, {provider.coverage.region}
      </p>

      <h3 className="provider-info-header">
        Contact Information 
        <button className="provider-info-toggle-button" onClick={() => setShowContactInfo(!showContactInfo)}>
      ...
      <h3 className="provider-info-header">
        ...
        <button className="provider-info-toggle-button" onClick={() => setShowAppointmentServices(!showAppointmentServices)}>
      ...
      {provider.hasResidentialServices && (
        <>
          <h3 className="provider-info-header">
            ...
            <button className="provider-info-toggle-button" onClick={() => setShowResidentialServices(!showResidentialServices)}>
      ...
      <h3 className="provider-info-header">
        Funding Sources
        <button className="provider-info-toggle-button" onClick={() => setShowFundingSources(!showFundingSources)}>
        ...
        <ul className="provider-info-funding">
          {provider.fundingSources.map((source, index) => (
            <li key={index}>
              ...
              {source.yearlyAmount.toLocaleString()} (Services: {source.servicesCovered.join(", ")})
            </li>
          ))}
        </ul>
      )}
